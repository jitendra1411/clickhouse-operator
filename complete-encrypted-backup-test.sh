...
METADATA=$(kubectl exec -n $NAMESPACE $POD_NAME -c clickhouse-backup -- cat /var/lib/clickhouse/backup/$BACKUP_DATE/metadata/$DATABASE_NAME/$TABLE_NAME.json 2>/dev/null)
echo "  📄 Table metadata:"
echo "$METADATA" | jq '.' 2>/dev/null || echo "$METADATA"

# Check if backup captured data files
DATA_FILES=$(kubectl exec -n $NAMESPACE $POD_NAME -c clickhouse-backup -- find /var/lib/clickhouse/backup/$BACKUP_DATE -type f -name "*.bin" | wc -l 2>/dev/null)
echo "  📁 Data files in backup: $DATA_FILES"
...
if echo "$RESTORE_RESULT" | grep -q "done.*operation=restore"; then
    ...
    # Show sample restored data
    echo "  📋 Sample restored data:"
    kubectl exec -n $NAMESPACE $POD_NAME -c clickhouse-pod -- clickhouse-client --query="SELECT id, user_name, email, balance FROM ${DATABASE_NAME}_restored.$TABLE_NAME ORDER BY id LIMIT 5" 2>/dev/null
...
echo ""
echo "🔹 Step 9: Configuration Analysis & Recommendations..."
echo "  🔧 Current clickhouse-backup configuration:"
kubectl exec -n $NAMESPACE $POD_NAME -c clickhouse-backup -- clickhouse-backup print-config 2>/dev/null | grep -E "(freeze_by_part|use_embedded_backup_restore|backup_mutations)" | head -3
...
echo "⚠️  DATA BACKUP: NEEDS CONFIGURATION ADJUSTMENT"
echo ""
echo "🔧 SOLUTION FOR DATA BACKUP:"
echo "1. The backup tool is correctly using FREEZE/UNFREEZE operations"
echo "2. Schema and structure are backed up properly"
echo "3. For complete data backup, consider:"
echo "   - Ensuring sufficient data volume (1000+ rows tested)"
echo "   - Using freeze_by_part=true in configuration"
echo "   - Enabling use_embedded_backup_restore=true"
echo "   - Checking backup_mutations=true"
echo ""
